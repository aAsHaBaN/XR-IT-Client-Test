...
export class AuthService {
    ...
    async validateUserCredentials(username: string): Promise<{ hash: string; salt: string; configuration_id: string, lab_id: string } | null> {
        try {
            const result = await this.client.query(
                "SELECT hash, salt, configuration_id, lab_id FROM users WHERE username = $1",
                [username]
            );
