...
/* 
  Loads and returns all Node configurations, accepts a configuration path (used for development for default configurations)
  otherwise, we use the default configuration location at config > xrit-node-config.json
*/
const getNodeConfigs = async function (config_path?: string): Promise<INodeConfig[]> {
...
*/
const getConfig = async function (id: string, config_path?: string): Promise<INodeConfig | undefined> {
  const vpn_configs = await getNodeConfigs(config_path);
...
/* 
  ...
  location at config > xrit-node-config.json
*/
const writeConfig = async function (config: INodeConfig, config_path?: string): Promise<INodeConfig> {
  if (!config.id) config.id = crypto.randomUUID();;

  var node_configs = await getNodeConfigs(config_path);
...
/* 
  ...
  location at config > xrit-node-config.json
*/
const removeConfig = async function (id: string, config_path?: string): Promise<Boolean> {
  const node_configs = await getNodeConfigs(config_path);
...
export { getNodeConfigs, getConfig, writeConfig, removeConfig };
