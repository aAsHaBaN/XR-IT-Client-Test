...
import runPowershell from "../../core/utils/powershell.js";

const MVN_LAUNCH_SCRIPT_PATH = build_path("/src/supported-services/scripts/MVN/LaunchMVN.ps1");
const MVN_STATUS_SCRIPT_PATH = build_path("/src/supported-services/scripts/MVN/GetMVNStatus.ps1");
const MVN_DEFAULT_REMOTE_COMMAND_PORT = 6004;
const MAX_UDP_TIMEOUTS = 3;

// Executes a powershell script to determine if MVN is running
...
// Executes a powershell script to launch MVN
export async function launchMVN() {
  return await runPowershell(MVN_LAUNCH_SCRIPT_PATH, []);
}

// Helper function to build XML requests
function buildXMLRequest(requestType: string, target: { ip: String, port: Number }): string {
...
export async function addNetworkStreamingTarget(target: { ip: String, port: Number }) {
...
export async function removeNetworkStreamingTarget(target: { ip: String, port: Number }) {
